...
namespace UnityExplorerPlus.Widgets.tk2d
{
	...
	internal class tk2dClipDumpWidget : Texture2DWidget
	{
		...
		public override GameObject CreateContent(GameObject uiRoot)
		{
			...
			Traverse.Create(this).Field("savePathInput").GetValue<InputFieldRef>().Transform.parent.gameObject.SetActive(false);
			GameObject textureViewerRoot = Traverse.Create(this).Field("textureViewerRoot").GetValue<GameObject>();
			...
			this.progressLabel = UIFactory.CreateLabel(playerRow, "ProgressLabel", "0 / 0", TextAnchor.MiddleLeft, default(Color), true, 14);
			...
			Text saveLabel = UIFactory.CreateLabel(saveRow, "SpriteSaveLabel", "Save Clip:", TextAnchor.MiddleLeft, default(Color), true, 14);
		...
		private IEnumerator PlayClipCoroutine()
		{
			...
			do
			{
				...
				flag2 = (wrapMode2 == null || wrapMode2 == tk2dSpriteAnimationClip.WrapMode.LoopSection || wrapMode2 == tk2dSpriteAnimationClip.WrapMode.RandomLoop);
				if (!true)
				{
				}
			}
		...
		public override void OnBorrowed(object target, Type targetType, ReflectionInspector inspector)
		{
			...
			this.savePlus.Text = Path.Combine(UnityExplorer.Config.ConfigManager.Default_Output_Path.Value, "tk2dSpriteAnimationClip-" + this.clip.name);
			this.ResetProgressLabel();
		}
