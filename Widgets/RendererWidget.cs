...
namespace UnityExplorerPlus.Widgets
{
	...
	internal class RendererWidget : Texture2DWidget
	{
		...
		{
			yield return null;
			GameObject root = this.textureViewerRoot;
			for (;;)
			{
		...
		public override GameObject CreateContent(GameObject uiRoot)
		{
			...
			GameObject saveRow = this.textureViewerRoot.transform.GetChild(0).gameObject;
			...
			Toggle o_toggle;
			Text text;
			GameObject toggle = UIFactory.CreateToggle(saveRow, "AutoRefresh", ref o_toggle, ref text, default(Color), 20, 20);
